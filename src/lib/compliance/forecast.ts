...
export function forecastRoute(planned: PlannedStop[], days: WorkDay[], settings: Settings, active: string[] = []): ScheduledStop[] {
  ...
  const estimates = (code: string, entries: WorkDay[], through: string) => {
    ...
    return { forecast: true, stateWages: hours * settings.salary / 2080, annualTotalWages: settings.salary,
